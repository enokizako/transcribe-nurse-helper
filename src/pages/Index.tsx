...
const defaultPrompt = `渡されたテキストを元に看護記録をSOAP形式にまとめてください。
SOAP形式の記載方法は下記に従ってください。
SOAPすべてを埋める必要はなく、必要なもののみ書き出してください。
Sは主観的データ(Subjective Data)
　患者さんの発した言葉。要約してもよいが、患者さんの発言以外を記録するのはNG!　例) 分かりました。 OKです。 など
...
const Index: React.FC = () => {
  ...
  useEffect(() => {
    const checkMicrophonePermission = async () => {
      try {
        ...
        toast.error('マイクへのアクセスに失敗しました。ブラウザの設定を確認してください。');
      }
    };
    ...
    if (TranscriptionService.isAvailable()) {
      ...
      toast.error('お使いのブラウザは音声認識をサポートしていません。Chrome または Edge の最新版をお使いください。');
    }
  }, []);
  ...
  const formatWithSOAP = (text: string, soapPrompt: string): string => {
    ...
    const subjectiveMatches = text.match(/「.*?」|『.*?』|患者[はが].*?(言った|述べた|話した)/g);
    ...
    const objectiveMatches = text.match(/\d+度|\d+\.?\d*[%％]|フェイススケール\d+|ブリストルスケール\d+型|バイタル|血圧|脈拍|SPO2|呼吸数/g);
    ...
    if (text.includes('考えられる') || text.includes('判断') || text.includes('アセスメント')) {
      sections.push(`【A: アセスメント】\n看護師の観察と評価に基づき、患者の状態を分析しました。`);
    }
    
    // Pセクション（計画）：「今後」「計画」などの表現を探す
    if (text.includes('今後') || text.includes('計画') || text.includes('予定')) {
